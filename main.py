...
def save_store():
    ...
    # 1. 先从 GCS 下载最新的数据（这一步比较慢，不需要加锁）
    store_info = get_store_info()

    try:
        # 2. 【核心修改】使用锁保护 traffic 的读取和清零
        # 这是一个原子操作：把当前的流量拿出来，立刻把计数器归零
        # 此时如果有其他线程想修改 traffic，会被阻塞，直到这里执行完
        current_period_traffic = 0
        with traffic_lock:
            current_period_traffic = traffic
            traffic = 0
        
        # 3. 将刚才取出的流量加到总数中
        # 这里的 store_info 是函数内的局部变量，修改它不需要加锁
        store_info["traffic"] = store_info.get("traffic", 0) + current_period_traffic
        
        # 4. 上传回 GCS（这一步最耗时，但因为是在锁外面执行，不会卡住其他请求）
        ...
        # 如果上传失败，理论上刚才清零的流量（current_period_traffic）可能会丢失。
        # 为了防止逻辑过于复杂导致死锁，这里选择暂不回滚。
        # 在 GAE 环境下，GCS 写入失败的概率较低。
...
def count_traffic(add_traffic):
    ...
    # 核心修改：加上这行代码
    # 任何线程运行到这里，必须先拿到锁才能进去。
    # 如果锁被别人拿了，就乖乖排队等，直到别人释放。
    # 这样就保证了 traffic += add_traffic 永远不会被覆盖。
    with traffic_lock:
        traffic += add_traffic
...
def graceful_shutdown(signum, frame):
    logging.warning("收到 SIGTERM 信号，实例即将关闭！正在进行最后一次数据保存...")
    # 强制保存数据，不管 timer 还没到时间
    save_store()
    logging.warning("最后一次保存完成，再见。")
    sys.exit(0)

# 注册信号监听：当收到 SIGTERM (15) 时，执行 graceful_shutdown 函数
